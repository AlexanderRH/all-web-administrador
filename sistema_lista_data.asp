...
on error resume next
	...
	cont = 0
	if not rs.eof then
		if orden_ca = 1 then rs.sort = "idsistema " & orden_ad
		if orden_ca = 2 then rs.sort = "nombre " & orden_ad
		if orden_ca = 3 then rs.sort = "descripcion " & orden_ad
	end if
...
on error resume next
	...
	cmd.ActiveConnection = cn 
	cmd.CommandType= adCmdStoredProc
	cmd.CommandText = "USP_Sistema_Eliminar"
