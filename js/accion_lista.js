...
function Eliminar(idaccion)
{
	try
	{
		...
		var url = "accion_lista_data.asp?token=" + token +
										"&accion=eliminar" +
										"&idaccion=" + idaccion
...
function Buscar(verificar)
{
	try
	{
		...
		var bfiltro = document.getElementById("txtFiltro").value
		var divProceso = document.getElementById("divProceso")
		var divGrillaAccion = document.getElementById("divGrillaAccion")
		...
		if(verificar == 1)
			if(document.getElementById("txtGrillaAccionURL").value == url)
				return false
		...
		ajax.onreadystatechange=
			...
			{ 
				...
				if (ajax.readyState == 4)
				{
					document.getElementById("txtGrillaAccionURL").value = url
					divProceso.style.display = "none"
					divGrillaAccion.innerHTML = ajax.responseText
					//BloquearControles(false);
				}
			}
		ajax.send(null);
