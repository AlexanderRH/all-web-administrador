...
'if busq = 0 then
	url = "accion_lista_data.asp?token=" & session.SessionID & "&accion=lista_accion&campo=" & campo & "&filtro=" & filtro
'end if
%>
	<input id="txtGrillaAccionURL" name="txtGrillaAccionURL" type="hidden" value="<%=url%>"/>

		<div style="width:100%; height:30px; font-size:17px; font-weight:bold; color:#3366CC;">
			<div class="pagina_titulo">Acci�n - Lista</div>
		...
		<div style="width:100%; height:40px;">
			...
			<div class="ventana_fila">
				...
				<div class="ventana_dato">
					<select id="cboCampoFiltro" class="combo" style="width:90px" onclick="Buscar(1);" onChange="Buscar(1);" onBlur="Buscar(1);">
						<option value="1" <% if campo = 1 then response.Write("selected=""selected""") end if%>>C�digo</option>
		...
		<div id="divGrillaAccion" style="background-color:white; width:935px; height:100%; overflow:auto; " >
<%
		'if busq = 0 then
